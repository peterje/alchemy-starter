...
- Documents and decks are versioned files. Derive a new file kind's schemas with `VersionedFile` in `packages/contract/src/versioning.ts`, give it an object that declares its store with `VersionedStore.Service` and provides the store's layer, and add it to the user's files.
