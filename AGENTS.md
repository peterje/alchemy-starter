...
- Documents and decks are versioned files. Derive a new file kind's schemas with `VersionedFile` in `packages/contract/src/versioning.ts`, give it an object that opens `openVersionedStore`, and add it to the user's files.
