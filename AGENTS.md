...
- Objects are the consistency boundary and expose RPC methods only, never `fetch`. Put state that must be read and written together in one object. Objects raise typed errors; the Worker decodes them back through their schema after the stub. Cross-object writes are idempotent.
