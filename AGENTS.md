...
- `apps/website/src/store.ts`, `notes.ts`, and `api.ts` are the contract. The browser imports them, so they must never import `alchemy`, `cloudflare:workers`, or `worker.ts`. The Vite build fails if they do.
- A feature is one contract module with an `HttpApiGroup` added in `api.ts`, one handlers block in the Durable Object in `worker.ts`, and one route under `routes/`. Every group shares the user's SQLite database and the `migrations` list.
