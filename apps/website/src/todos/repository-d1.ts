...
const getTodo = (id: string) =>
  Effect.tryPromise({
    try: () =>
      env.TODOS.prepare("SELECT id, title, completed FROM todos WHERE id = ?")
        .bind(id)
        .first<TodoRow>(),
...
export const TodoRepositoryD1 = Layer.succeed(TodoRepository, {
  list: () =>
    Effect.tryPromise({
      try: async () => {
        const result = await env.TODOS.prepare(
          "SELECT id, title, completed FROM todos ORDER BY created_at ASC, id ASC",
        ).all<TodoRow>();
  ...
  create: (input) => {
    ...
    return Effect.tryPromise({
      try: () =>
        env.TODOS.prepare("INSERT INTO todos (id, title, completed) VALUES (?, ?, ?)")
  ...
  update: (id, input) =>
    Effect.gen(function* () {
      ...
      yield* Effect.tryPromise({
        try: () =>
          env.TODOS.prepare("UPDATE todos SET title = ?, completed = ? WHERE id = ?")
  ...
  remove: (id) =>
    Effect.gen(function* () {
      ...
      yield* Effect.tryPromise({
        try: () => env.TODOS.prepare("DELETE FROM todos WHERE id = ?").bind(id).run(),
