...
test("a document is created from the library, edited block by block, and survives reloads", async ({
  ...
  await expect(page.locator("p", { hasText: "Start writing." })).toBeVisible();

  await page.getByLabel("New paragraph").fill("First question");
  await page.getByRole("button", { name: "Add paragraph" }).click();
  await expect(page.locator("p", { hasText: "First question" })).toBeVisible();
  ...
  await expect(page.locator("p", { hasText: "Start here." })).toBeVisible();
...
test("a deck is created from the library and gains and loses slides", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Create file" }).click();
  await expect(page.getByRole("region", { name: title })).toBeVisible();
  await expect(page.getByRole("article", { name: "Slide 1" })).toBeVisible();

  await page.getByLabel("New slide").fill("Objectives");
  await page.getByRole("button", { name: "Add slide" }).click();
  await expect(page.getByRole("article", { name: "Slide 2" })).toContainText("Objectives");

  await page
    .getByRole("article", { name: "Slide 1" })
    .getByRole("button", { name: "Delete slide" })
    .click();
  await expect(page.getByRole("article", { name: "Slide 2" })).toHaveCount(0);
  await expect(page.getByRole("article", { name: "Slide 1" })).toContainText("Objectives");
});
